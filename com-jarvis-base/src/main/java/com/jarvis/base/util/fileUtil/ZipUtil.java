...
public class ZipUtil {
	 ...
	 * @throws Exception
	 */
	public static HashMap<String, String> UnZipFolder(String zipFileString,
			String outPathString) throws Exception {
System.out.println("zipFileString:"+zipFileString+"outPathString:"+outPathString);
		...
		while ((zipEntry = inZip.getNextEntry()) != null) {
			...
			if (szName.indexOf("/") > -1) {
				String[] splitPath = szName.split("/");
				for (int i = 0; i < splitPath.length - 1; i++) {
					java.io.File folder = new java.io.File(outPathString
							+ java.io.File.separator + splitPath[i]);
			...
			if (zipEntry.isDirectory()) {
				...
				java.io.File file = new java.io.File(outPathString
						+ java.io.File.separator + szName);
				retVal.put(szName, outPathString + java.io.File.separator
						+ szName);
				file.createNewFile();
	...
	private static void ZipFiles(String folderString, String fileString,
		...
		// android.util.Log.v("XZip",
		// "ZipFiles(String, String, ZipOutputStream)");
	...
	public static void main(String[] args) throws Exception {
		...
		// ZipFolder("O:/zips/","O:/jj.zip");
		// CopyFile(new File("O:/jj.zip"), new File("O:/kk.zip"));

		UnZipFolder("O:/tmp.zip", "O:/");
	}
}
